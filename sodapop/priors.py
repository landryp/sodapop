...
def flat_mcqdet_quad_dL_flat_chieff(m1,m2,dL,chi1,chi2,cost1,cost2):

	chimax = 0.89
	chieff = (chi1*cost1+(m2/m1)*chi2*cost2)/(1.+(m2/m1))

	if (m1 < m2) or (np.abs(chi1) > chimax) or (np.abs(chi2) > chimax) or (np.abs(cost1) > 1.) or (np.abs(cost2) > 1.): val = 0.
	elif np.abs(chieff) <= chimax*(1.-(m2/m1))/(1.+(m2/m1)): val = dL**2*(1.+dL_to_z(dL))*(m1*m2)**0.6/(m1**2*(m1+m2)**0.2)
	else: val = (1.-(1.-(m2/m1)/(1.+(m2/m1))))/(1.-np.abs(chieff)/chimax)*dL**2*(1.+dL_to_z(dL))*(m1*m2)**0.6/(m1**2*(m1+m2)**0.2)
