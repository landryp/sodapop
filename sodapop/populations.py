...
def unif_m1m2_common_chi1chi2(m1,m2,chi1,chi2,lambdaa): # uniform distribution in component masses and spins, subject to common-EOS dependent maximum spin
	...
	chi1max = np.array([min(lambdaa[3],chi1max0) for chi1max0 in chi1max])
	chi2max = np.array([min(lambdaa[3],chi2max0) for chi2max0 in chi2max])
	
	common1 = (lambdaa[2],lambdaa[3])
	common2 = (lambdaa[2],lambdaa[3])
	
	p = unif_m1m2(m1,m2,lambdaa)*unif_common_chi(chi1,chi1max,common1)*unif_common_chi(chi2,chi2max,common2)
...
def peakcut_m1m2_common_chi1chi2(m1,m2,chi1,chi2,lambdaa): # uniform distribution in component masses and spins, subject to common-EOS dependent maximum spin
	...
	chi1max = np.array([min(lambdaa[5],chi1max0) for chi1max0 in chi1max])
	chi2max = np.array([min(lambdaa[5],chi2max0) for chi2max0 in chi2max])
	
	common1 = (lambdaa[4],lambdaa[5])
	common2 = (lambdaa[4],lambdaa[5])
	
	p = peakcut_m1m2(m1,m2,lambdaa)*unif_common_chi(chi1,chi1max,common1)*unif_common_chi(chi2,chi2max,common2)
...
def bimodcut_m1m2_common_chi1chi2(m1,m2,chi1,chi2,lambdaa): # uniform distribution in component masses and spins, subject to common-EOS dependent maximum spin
	...
	chi1max = np.array([min(lambdaa[8],chi1max0) for chi1max0 in chi1max])
	chi2max = np.array([min(lambdaa[8],chi2max0) for chi2max0 in chi2max])
	
	common1 = (lambdaa[7],lambdaa[8])
	common2 = (lambdaa[7],lambdaa[8])
	
	p = bimodcut_m1m2(m1,m2,lambdaa)*unif_common_chi(chi1,chi1max,common1)*unif_common_chi(chi2,chi2max,common2)
	
	return np.where(m1 < m2, z, p)

def unif_m1m2_power_chi1_power_chi2(m1,m2,chi1,chi2,lambdaa): # bimodal distribution in component masses, with one fast and one slow spin
	...
	p = unif_m1m2(m1,m2,lambdaa)*power_chi1_power_chi2(chi1,chi2,lambdaa[2:])
	
	return np.where(m1 < m2, z, p)
	
def peakcut_m1m2_power_chi1_power_chi2(m1,m2,chi1,chi2,lambdaa): # bimodal distribution in component masses, with one fast and one slow spin
	...
	p = peakcut_m1m2(m1,m2,lambdaa)*power_chi1_power_chi2(chi1,chi2,lambdaa[4:])
	
	return np.where(m1 < m2, z, p)
	
def bimodcut_m1m2_power_chi1_power_chi2(m1,m2,chi1,chi2,lambdaa): # bimodal distribution in component masses, with one fast and one slow spin
	...
	p = bimodcut_m1m2(m1,m2,lambdaa)*power_chi1_power_chi2(chi1,chi2,lambdaa[7:])
...
def unif_m1m2_common_chi1chi2_common_Lambda1Lambda2(m1,m2,chi1,chi2,Lambda1,Lambda2,lambdaa): # uniform mass model with random pairing, uniform uncorrelated spins and common-EOS approximated tidal deformabilities
	...
	p = unif_m1m2_common_Lambda1Lambda2(m1,m2,Lambda1,Lambda2,lambdaa[:2]+lambdaa[4:])*unif_common_chi(chi1,chi1max,common1)*unif_common_chi(chi2,chi2max,common2)
...
def peakcut_m1m2_common_chi1chi2_common_Lambda1Lambda2(m1,m2,chi1,chi2,Lambda1,Lambda2,lambdaa): # Gaussian mass model with random pairing, uniform uncorrelated spins and common-EOS approximated tidal deformabilities
	...
	p = peakcut_m1m2_common_Lambda1Lambda2(m1,m2,Lambda1,Lambda2,lambdaa[:4]+lambdaa[6:])*unif_common_chi(chi1,chi1max,common1)*unif_common_chi(chi2,chi2max,common2)
...
def bimodcut_m1m2_common_chi1chi2_common_Lambda1Lambda2(m1,m2,chi1,chi2,Lambda1,Lambda2,lambdaa): # bimodal mass model with random pairing, uniform uncorrelated spins and common-EOS approximated tidal deformabilities
	...
	p = bimodcut_m1m2_common_Lambda1Lambda2(m1,m2,Lambda1,Lambda2,lambdaa[:7]+lambdaa[9:])*unif_common_chi(chi1,chi1max,common1)*unif_common_chi(chi2,chi2max,common2)

	return np.where((m1 < m2) | (Lambda2 < Lambda1), z, p)

def unif_m1m2_power_chi1_power_chi2_common_Lambda1Lambda2(m1,m2,chi1,chi2,Lambda1,Lambda2,lambdaa): # uniform mass model with random pairing, power-law uncorrelated spins and common-EOS approximated tidal deformabilities
	...
	p = unif_m1m2_common_Lambda1Lambda2(m1,m2,Lambda1,Lambda2,lambdaa[:2]+lambdaa[8:])*power_chi1_power_chi2(chi1,chi2,lambdaa[2:])

	return np.where((m1 < m2) | (Lambda2 < Lambda1), z, p)
	
def peakcut_m1m2_power_chi1_power_chi2_common_Lambda1Lambda2(m1,m2,chi1,chi2,Lambda1,Lambda2,lambdaa): # peak mass model with random pairing, power-law uncorrelated spins and common-EOS approximated tidal deformabilities
	...
	p = peakcut_m1m2_common_Lambda1Lambda2(m1,m2,Lambda1,Lambda2,lambdaa[:4]+lambdaa[10:])*power_chi1_power_chi2(chi1,chi2,lambdaa[4:])

	return np.where((m1 < m2) | (Lambda2 < Lambda1), z, p)

def bimodcut_m1m2_power_chi1_power_chi2_common_Lambda1Lambda2(m1,m2,chi1,chi2,Lambda1,Lambda2,lambdaa): # bimodal mass model with random pairing, power-law uncorrelated spins and common-EOS approximated tidal deformabilities
	...
	p = bimodcut_m1m2_common_Lambda1Lambda2(m1,m2,Lambda1,Lambda2,lambdaa[:7]+lambdaa[13:])*power_chi1_power_chi2(chi1,chi2,lambdaa[7:])
...
def peakcut_m1m2_dns_spins_common_Lambda1Lambda2(m1,m2,chi1,chi2,cost1,cost2,Lambda1,Lambda2,lambdaa): # peak mass model with random pairing, power-law uncorrelated spins and common-EOS approximated tidal deformabilities
...
def bimodcut_m1m2_dns_spins_common_Lambda1Lambda2(m1,m2,chi1,chi2,cost1,cost2,Lambda1,Lambda2,lambdaa): # bimodal mass model with random pairing, power-law uncorrelated spins and common-EOS approximated tidal deformabilities
	...
	p = bimodcut_m1m2_common_Lambda1Lambda2(m1,m2,Lambda1,Lambda2,lambdaa[:7]+lambdaa[13:])*power_chi1_power_chi2(chi1,chi2,lambdaa[7:])*gaussian(np.arccos(cost1),dns_t1)/np.sin(np.arccos(cost1)) #FIXME: singular when t1 = 0 
